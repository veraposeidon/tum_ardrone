...
void EstimationNode::navdataCb(const ardrone_autonomy::NavdataConstPtr navdataPtr) {
    ...
    // 将导航数据从驱动ardrone_autonomy那纠正回到Drone发布的状态
    ...
    if (logfileIMU != NULL) {
        ...
        if (logfileIMU != NULL)
            (*logfileIMU) << getMS(lastNavdataReceived.header.stamp) << " " << lastNavdataReceived.tm << " " <<
                          ...
                          lastNavdataReceived.pressure << " " << 0 << " " << 0 << " " << 0 << " "
                          <<    // control: roll pitch gaz yaw.
                          pingNav << " " << pingVid << "\n";
...
void EstimationNode::Loop() {
    ...
    while (nh_.ok()) {
        ...
        // 如果PTAM还没有运行，则使用addFakePTAMObservation函数进行PTAM假操作，避免滤波器待处理数据队列急剧增大
